...
pub fn main() !void {
    ...
    const queue_binary = try allocator.alloc(u8, working_directory.len + relative_queue_binary.len);
    defer allocator.free(queue_binary);

    @memcpy(queue_binary[0..working_directory.len], working_directory);
    @memcpy(queue_binary[working_directory.len..], relative_queue_binary);

    const relative_worker_binary = "/zig-out/bin/rbs-worker";
    const worker_binary = try allocator.alloc(u8, working_directory.len + relative_worker_binary.len);
    defer allocator.free(worker_binary);

    @memcpy(worker_binary[0..working_directory.len], working_directory);
    @memcpy(worker_binary[working_directory.len..], relative_worker_binary);

    // FIXME(garrett): Remove this once all settled
    std.debug.print("Expected queue binary: {s}\n", .{queue_binary});
    std.debug.print("Expected worker binary: {s}\n", .{worker_binary});

    const queue_pid = c.fork();

    // TODO(garrett): Handle -1 ret val indicating a failure to fork
    if (queue_pid == 0) {
        std.debug.print("Hello from child 1!\n", .{});
        std.process.exit(0);
    }
    ...
    // TODO(garrett): Handle -1 ret val indicating a failure to fork
    if (worker_pid == 0) {
        std.debug.print("Hello from child 2!\n", .{});
        std.process.exit(1);
    }

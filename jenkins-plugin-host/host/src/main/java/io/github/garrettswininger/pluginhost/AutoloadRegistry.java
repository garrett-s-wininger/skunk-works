...
record Registry(List<ManagementLinkProxy> managementLinks) {
  // TODO(garrett): Registry production from JAR contents
}

record RegisteredJar(byte[] digest, URLClassLoader loader, Registry registry) {
  public static RegisteredJar fromFile(File file) throws IOException {
    ...
    // TODO(garrett): Registry production from JAR contents
    final var registry = new Registry(new ArrayList<>());
    final var loader =
...
class AutoloadRegistry {
  ...
  private Optional<DynamicPlugin<?, ?>> pluginFromEntry(ClassLoader loader, JarEntry entry) {
  ...
  // private RegisteredJar createRegistrationFromJar(File file) {
  ...
  //         .map(entry -> pluginFromEntry(loader, entry))
  ...
  void register(Path pluginPath) {
    ...
    //                           LOGGER.info(
    //                               String.format(
    //                                   "%s extension: %s successfully installed into the
    // instance",
